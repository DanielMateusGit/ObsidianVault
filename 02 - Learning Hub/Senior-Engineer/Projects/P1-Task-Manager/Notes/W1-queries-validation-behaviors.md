...
### Flusso completo del ValidationBehavior

```
1. Command arriva al ValidationBehavior.Handle()
2. Handle() prende la request (il Command)
3. Esegue tutti i Validator registrati per quel Command
4. Se errori → throw ValidationException (l'Handler non viene MAI toccato)
5. Se OK → next() → esegue l'Handler del Command → ritorna il risultato (TResponse)
```

Il `ValidationBehavior` è il **bouncer**: controlla il biglietto (validazione) prima di far entrare (next → Handler).

**Catena ereditarietà Validator:**
```
IValidator<T>                  ← interfaccia (usata dal Behavior)
  └── AbstractValidator<T>     ← classe base (FluentValidation)
        └── CreateTaskItemCommandValidator  ← il nostro validator concreto
```

**Chi trova cosa:**
| Cosa | Chi lo trova |
|------|-------------|
| Handler per Command/Query | MediatR (automatico) |
| Validator per Command | DI Container → ValidationBehavior |

**next() non è una funzione da ritornare — è una funzione da ESEGUIRE:**
- `next` → il delegate (la funzione)
- `next()` → la chiamata → restituisce `Task<TResponse>`
- `await next()` → il risultato → un `TResponse` (es. `Guid`)
...
### Marker Interface — Behavior solo per certi Command

Se vuoi un Behavior che si attiva **solo** per alcuni Command, usi una **marker interface** (interfaccia vuota che "etichetta"):

```csharp
// 1. Crea il marker
public interface IRequireLogging { }

// 2. Tagghi i command che vuoi
public record CreateTaskItemCommand(string Title, Priority Priority)
    : IRequest<Guid>, IRequireLogging;  // ← taggato

public record DeleteTaskItemCommand(Guid TaskItemId)
    : IRequest<Unit>;  // ← NON taggato

// 3. Nel Behavior, aggiungi il constraint
public class LoggingBehavior<TRequest, TResponse>
    : IPipelineBehavior<TRequest, TResponse>
    where TRequest : IRequest<TResponse>, IRequireLogging  // ← solo chi ha il marker
```

MediatR inietta il Behavior **solo** per i command che implementano il marker. Gli altri passano dritti.

**Perché non viola ISP?** Una marker interface non ha metodi — non forza nessuno a implementare nulla. Serve solo come "tag" per il DI.

**Quando usarla:** Per behavior selettivi (logging, caching, auditing). Il `ValidationBehavior` invece resta per tutti — la validazione è cross-cutting.
